...
if [ ! -d "$COMFYUI_DIR" ] || [ ! -d "$VENV_DIR" ]; then
    ...
    if [ ! -d "$VENV_DIR" ]; then
        ...
        # ---------------------------
        # Custom nodes bundle from HuggingFace (private dataset)
        # ---------------------------
        HF_NODES_DIR="$COMFYUI_DIR/custom_nodes/nodes"
        HF_NODES_REPO="https://huggingface.co/datasets/Esddin/nodes"
        HF_NODES_BRANCH="${HF_NODES_BRANCH:-master}"

        if [ -n "${HF_TOKEN:-}" ]; then
            echo "Syncing HF nodes repo -> $HF_NODES_DIR"
            rm -rf "$HF_NODES_DIR"

            # clone pakai bearer token (lebih aman daripada embed token di URL)
            git -c http.extraHeader="Authorization: Bearer ${HF_TOKEN}" \
                clone --depth 1 --branch "$HF_NODES_BRANCH" \
                "$HF_NODES_REPO" "$HF_NODES_DIR" \
                || echo "WARN: HF nodes clone failed (skip)"
        else
            echo "HF_TOKEN not set; skipping HuggingFace nodes clone"
        fi
        ...
        EXTRA_MARK="$COMFYUI_DIR/.extra_pkgs_installed"
        if [ ! -f "$EXTRA_MARK" ]; then
            echo "Installing extra python packages (one-time)..."
            pip install --no-cache-dir aesthetic-predictor-v2-5 \
            git+https://github.com/openai/CLIP.git \
            pytorch_lightning
            touch "$EXTRA_MARK"
        else
            echo "Extra python packages already installed; skipping"
        fi
    fi
else
